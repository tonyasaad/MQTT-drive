...
void MQTT_Connect(uint8* clientID ){
	...
	UART_sendString("AT+CIPSEND=18\r\n"); // we send the bytes numbers of the packet we are going to send
			_delay_ms(2000);
			_delay_ms(2000);

uint8 MQTT_ARR[18]; //MQTT_ARR[255]; we can put 255 instead of 14+num
	MQTT_ARR[0]=	0x10;
	MQTT_ARR[1]=	(16);
	MQTT_ARR[2]=	0x00;
	MQTT_ARR[3]=	0x04;
	...
	MQTT_ARR[12]=	0x00;
	MQTT_ARR[13]=	0x04;
	MQTT_ARR[14]=	clientID[0];
	MQTT_ARR[15]=	clientID[1];
	MQTT_ARR[16]=	clientID[2];
	MQTT_ARR[17]=	clientID[3];
